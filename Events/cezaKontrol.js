...
module.exports = async member => {
    ...
    let embed = new Discord.MessageEmbed().setColor(`RANDOM`).setAuthor({ name: member.guild.name, iconURL: member.guild.iconURL({ dynamic: true }) });

    cdata.forEach(async (data, index) => {
        if (data.Durum === true) {
            member.roles.add(ayar.roles.muteRole);
            if (clog) clog.send({ embeds: [embed.setDescription(`${member} Adlı kullanıcının aktif bir **[MUTE]** cezası varken sunucudan çıktığı için muteli rolü verildi.`)] });
        }
    })
    ...
    vdata.forEach(async (data, index) => {
        if (data.Durum === true) {
            ...
            if (vlog) vlog.send({ embeds: [embed.setDescription(`${member} Adlı kullanıcının aktif bir **[VOICE-MUTE]** cezası varken sunucudan çıktığı için muteli rolü verildi.`)] });
        }
    })
    ...
    jdata.forEach(async (data, index) => {
        if (data.Durum === true) {
            ...
            if (jlog) jlog.send({ embeds: [embed.setDescription(`${member} Adlı kullanıcının aktif bir **[JAIL]** cezası varken sunucudan çıktığı için jail rolü verildi.`)] });
        }
    })
};
