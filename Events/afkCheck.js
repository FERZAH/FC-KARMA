    ...
    if (member) {
        ...
        if (memberKontrol) {
            memberData.forEach(async (d, index) => {
                message.channel.send({ embeds: [embed.setDescription(`${member} Adlı kullanıcı **${d.Reason}** sebebiyle ${moment.duration(Date.now() - d.Tarih).format("D [Gün], H [Saat], m [Dakika], s [Saniye]")} önce AFK oldu!`)] });
            });
        }
    }

    if (kontrol) {
        message.channel.send({ embeds: [embed.setDescription(`${message.member} Artık AFK değilsin!`)] }).then(m => m.delete({ timeout: 7000 }));
