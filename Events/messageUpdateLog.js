...
module.exports = async (old, nev) => {
        ...
        let embed = new MessageEmbed().setColor('RANDOM').setAuthor({ name: old.author.tag, iconURL: old.author.avatarURL({ dynamic: true }) });
        let entry = await old.guild.fetchAuditLogs({ type: 'MESSAGE_DELETE' }).then(audit => audit.entries.first())


        if (log) log.send({
                embeds: [embed.setDescription(`${nev.channel} Kanalında ${entry.executor} tarafından bir mesaj düzenlendi!
