...
module.exports.run = async (client, message, args, embed) => {
    if (ayar.roles.banStaff.some(r => !message.member.roles.cache.has(r)) && !message.member.permissions.has("ADMINISTRATOR")) return message.channel.send({ embeds: [embed.setDescription(`${message.author}, Bu komutu kullanmak için yeterli yetkiye sahip değilsin!`)] }).sil(7);
    ...
    if (!member || !sebep) return message.channel.send({ embeds: [embed.setDescription(`${message.member}, Hatalı kullanım. Örnek: **${ayar.bot.botPrefix}ban @Cross/ID [Reason]**`)] }).sil(7)
    ...
    if (limitData && limitData.Count >= ayar.guild.banLimit) return message.channel.send({ embeds: [embed.setDescription(`${message.member}, Bu günlük ban limit sınırına ulaştın.`)] }).sil(7)
