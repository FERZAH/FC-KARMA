...
module.exports.run = async (client, message, args, embed) => {
    ...
    if (!member) return message.channel.send({ embeds: [embed.setDescription(`${message.member}, Bir kullanıcı etiketle`)] }).sil(7)
    ...
    if (!data || !data.length) return message.channel.send({ embeds: [embed.setDescription(`${member}, Kullanıcısına ait rol verisi bulunamadı.`)] }).sil(7)
...
${listed}`).setAuthor({ name: member.user.tag, iconURL: member.user.avatarURL({ dynamic: true }) })]
    }).sil(15);
};
exports.config = {
    name: "rollog",
