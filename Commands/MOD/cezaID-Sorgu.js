...
module.exports.run = async (client, message, args, embed) => {
    ...
    if (!args[0] || (args[0] && isNaN(args[0]))) return message.channel.send({ embeds: [embed.setDescription(`${message.author}, Hatalı kullanım; \n \`\`\`Örnek: ${this.config.usage}\`\`\``)] });
    let data = await penalData.find({ guildID: message.guild.id, cezaID: args[0] });
    if (!data.length) return message.channel.send({ embeds: [embed.setDescription(`Bu ID'ye ait veri bulunamadı.`)] });
    data.map((d, index) => {
...
Ceza Puanı: **${d.Point}**
    `)
            .setThumbnail(user ? user.user.avatarURL({ dynamic: true }) : message.author.avatarURL({ dynamic: true })).setAuthor({ name: user ? user.user.username : message.author.username, iconURL: user ? user.user.avatarURL({ dynamic: true }) : message.author.avatarURL({ dynamic: true }) })
    });
