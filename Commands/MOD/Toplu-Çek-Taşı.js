...
module.exports.run = async (client, message, args, embed) => {
    ...
    if (cmd == "taşı") {
        ...
        if (!channel) return message.channel.send({ embeds: [embed.setDescription(`${message.member}, Bir kanal ID belirt.`)] }).sil(7);
        members.forEach(async m => {
            m.voice.setChannel(channel.id).catch(e => { });
        })
        message.channel.send({ embeds: [embed.setDescription(`**${message.member.voice.channel.members.size}** Adet kullanıcı ${channel} kanalına taşındı!`)] }).sil(7);
    } else if (cmd == "çek") {
        if (!message.member.voice.channel) return message.react(ayar.emojis.no);
        if (!channel) return message.channel.send({ embeds: [embed.setDescription(`${message.member}, Bir kanal ID belirt.`)] }).sil(7);
        ...
        message.channel.send({ embeds: [embed.setDescription(`**${channel.members.size}** Adet kullanıcı ${message.member.voice.channel} kanalına taşındı!`)] }).sil(7);
