...
`)]
        ...
        message.channel.send({ embeds: [embed.setDescription(`${aktifunsesyt.size} Yetkiliye ses çağrısı yapılıyor.`).setColor('RANDOM')] }).then(async (msg) => {
            aktifunsesyt.array().forEach(async (cross, index) => {
                setTimeout(async () => {
                    msg.edit({ embeds: [embed.setDescription(`${cross} Yetkilisine özelden mesaj atıldı.`)] })
                    cross.send(`Aktifsin fakat seste değilsin lütfen ses kanalına gir.\n**${message.guild.name}**`).catch(err => message.channel.send({ content: `${cross} Aktifsin fakat seste değilsin lütfen ses kanalına gir.` }) && msg.edit({ embeds: [embed.setDescription(`${cross} kullanıcısına özelden mesaj gönderilemediği için kanala etiketlendi.`)] }))
                }, index * 1500)
            })
