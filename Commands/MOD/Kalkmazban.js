...
module.exports.run = async (client, message, args, embed) => {
    ...
    if (args[0] == "aç") {
        ...
        if (!member) return message.channel.send({ embeds: [embed.setDescription(`${message.member}, Geçerli bir üye belirtmelisin.`)] }).sil(7);
        if (!data || !data.Members.includes(member.id)) return message.channel.send({ embeds: [embed.setDescription(`Bu kullanıcının banı bulunmamakta.`)] }).sil(7);
        ...
        if (!sebep) return message.channel.send({ embeds: [embed.setDescription(`${message.member}, Geçerli bir sebep belirtmelisin.`)] }).sil(7);
