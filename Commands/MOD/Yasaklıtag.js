...
module.exports.run = async (client, message, args, embed) => {
    ...
    if (cmd === 'ekle') {
        ...
        if (!hedef || !reason) return message.channel.send({ embeds: [embed.setDescription(`Geçerli bir tag ve sebep belirtmelisin.`)] })
        if (data && data.some(s => s.Tag === hedef)) return message.channel.send({ embeds: [embed.setDescription(`${message.member}, Bu tag zaten yasaklı listede mevcut.`)] })
        let x = new tagData({
        ...
        x.save().then(s => {
            message.channel.send({ embeds: [embed.setDescription(`\`${hedef}\` tagı başarılı bir şekilde yasaklı listeye eklendi!`)] })
        ...
        if (!data) return message.channel.send({ embeds: [embed.setDescription(`Sunucuya ait yasaklı tag verisi bulunamadı!`)] })
...
`)]
        ...
        if (!data && !data.length) return message.channel.send({ embeds: [embed.setDescription(`Sunucuya ait yasaklı tag verisi bulunmadığından silme işlemi yapılamaz!`)] })
        let hedef = args[1]
        if (!hedef) return;
        if (!data.some(s => s.Tag === hedef)) return message.channel.send({ embeds: [embed.setDescription(`Bu tag yasaklı listede zaten bulunmamakta!`)] })

        await tagData.deleteOne({ Tag: hedef }).then(s => {
            message.channel.send({ embeds: [embed.setDescription(`\`${hedef}\` tagı yasaklı listeden kaldırıldı!`)] })
            message.guild.members.cache.filter(s => s.user.tag.toLowerCase().includes(hedef)).forEach(async m => {
