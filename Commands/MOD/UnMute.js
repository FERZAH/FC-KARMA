...
module.exports.run = async (client, message, args, embed) => {
    ...
    if (!member) return message.channel.send({ embeds: [embed.setDescription(`${message.member}, Hatalı kullanım. Örnek: **${ayar.bot.botPrefix}unmute @Cross/ID**`)] }).sil(7)
    ...
    if (durum || !durumData.length) return message.channel.send({ embeds: [embed.setDescription(`${member}, Adlı kullanıcının aktif bir **[CHAT-MUTE]** cezası bulunmamakta!`)] }).sil(7);
