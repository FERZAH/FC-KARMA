...
module.exports.run = async (client, message, args, embed) => {
    ...
    if (!member) return message.channel.send({ embeds: [embed.setDescription(`${message.member}, Hatalı kullanım. Örnek: **${ayar.bot.botPrefix}sicil @Cross/ID**`)] }).sil(7)
    let memberData = await penalData.find({ guildID: message.guild.id, userID: member.id })
    if (!memberData || !memberData.length) return message.channel.send({ embeds: [embed.setDescription(`${member}, Adlı kullanıcının sicil geçmişi bulunmamakta`)] })
    let list = memberData.length > 0 ? memberData.map((value, i) => `\`${i + 1}\` **[${value.Tip}]** <@!${value.adminID}> tarafından **${value.Reason}** sebebiyle ${value.Durum == true ? yes : no}`).join('\n') : ""
