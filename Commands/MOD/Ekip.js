...
module.exports.run = async (client, message, args, embed) => {
    ...
    if (cmd == "ekle") {
        ...
        if (!ekipTag) return message.channel.send({ embeds: [embed.setDescription(`Lütfen bir tag belirt.`)] }).sil(7);
        let kontrol = data.some(s => s.Tag == ekipTag);
        if (kontrol) return message.channel.send({ embeds: [embed.setDescription(`Bu tag zaten ekip olarak alınmış.`)] }).sil(7);
        new ekipData({ guildID: message.guild.id, adminID: message.author.id, Tag: ekipTag, Date: Date.now() }).save().then(async m => {
            let msg = await message.channel.send({ embeds: [embed.setDescription(`**${ekipTag}** tagı başarıyla ekip olarak alındı!`)] })
        ...
        > Sunucumuzda tagımızı alan bizi destekleyen ekip üyelerinin miktarı: \`${ekipuyeleri.filter(s => ayar.guild.tags.some(a => s.user.tag.toLowerCase().includes(a))).size}\`
        `)]
                }).sil(10);
            }, 5000)
        })
    } else if (cmd == "liste") {
        if (!data || !data.length) return message.channel.send({ embeds: [embed.setDescription(`Sunucuya ait ekip verisi bulunamadı!`)] }).sil(7);

        let listed = data.map(s => `> TAG: **${s.Tag}** Ekleyen: <@!${s.adminID}> Tarih: **${moment(s.Date).locale('tr').format('LLL')}**`).join('\n')
        message.channel.send({ embeds: [embed.setDescription(listed)] }).sil(15)
    } else if (cmd == "sil") {
        let hedefTag = args.slice(1).join(' ');
        if (!data.map(s => s.Tag).includes(hedefTag)) return message.channel.send({ embeds: [embed.setDescription(`Bu tag zaten ekip sisteminde bulunmamakta.`)] }).sil(7);
        await ekip.deleteOne({ Tag: hedefTag }).then(m => {
            message.channel.send({ embeds: [embed.setDescription(`${hedefTag} tagı ekip sisteminden kaldırıldı!`)] })
        }).catch(ee => { });
    } else if (cmd == "bilgi") {
        let hedefTag = args.slice(1).join(' ');
        if (!data.map(s => s.Tag).includes(hedefTag)) return message.channel.send({ embeds: [embed.setDescription(`Bu tag ekip sisteminde bulunmamakta.`)] }).sil(7);
