...
module.exports.run = async (client, message, args, embed) => {
    if (ayar.roles.vmuteStaff.some(s => !message.member.roles.cache.has(s)) && !message.member.permissions.has("ADMINISTRATOR")) return message.channel.send({ embeds: [embed.setDescription(`${message.author}, Bu komutu kullanmak için yeterli yetkiye sahip değilsin!`)] }).sil(7);
    let member = message.mentions.members.first() || message.guild.members.cache.get(args[0]);
    let zaman = args[1];
    let sebep = args.slice(2).join(' ')
    if (!member || !zaman || !sebep) return message.channel.send({ embeds: [embed.setDescription(`${message.member}, Hatalı kullanım. Örnek: **${ayar.bot.botPrefix}vmute @Cross/ID [Time] [Reason]**`)] }).sil(7)
    ...
    if (durum) return message.channel.send({ embeds: [embed.setDescription(`${member}, Adlı kullanıcının aktif bir **[VOİCE-MUTE]** cezası bulunmakta.`)] }).sil(7);
    ...
    if (member.permissions.has("ADMINISTRATOR")) return message.channel.send({ embeds: [embed.setDescription(`${message.member}, Yönetici bir kullanıcıya karışamam.`)] }).sil(7)
    ...
    if (memberData.some(s => s.Point >= 5)) {
        ...
        if (puanKontrol > 75) {
            ...
            return message.channel.send({ embeds: [embed.setDescription(`${member} Adlı kullanıcının ceza puan'ı **75** ve üstü olduğu için yasaklandı!`)] }).sil(7);
        }
    }
    ...
    setTimeout(async () => {
        ...
        data.forEach(async (d, index) => {
            if (d.Durum == true) {
                ...
                if (client.channels.cache.get(ayar.channels.vMuteLog)) client.channels.cache.get(ayar.channels.vMuteLog).send({ embeds: [embed.setDescription(`${member}, Adlı kullanıcının ses kanallarında susturması kalktı.`)] })
                d.Durum = false;
                d.save();
            } else {
                return;
