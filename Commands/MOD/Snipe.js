...
module.exports.run = async (client, message, args, embed) => {
    ...
    if (snipeData.length <= 0) return message.channel.send({ embeds: [embed.setDescription(`Sunucu mesaj verisi bulunamadı!`)] }).sil(7);
    if (!sıra) {
...
${list.slice(snipeData.length - 1, snipeData.length)}
    ...
    if (!numbers.some(s => sıra.includes(s))) return message.channel.send({ embeds: [embed.setDescription(`Lütfen geçerli bir rakam giriniz.`)] })
    if (sıra) {
        return message.channel.send({ embeds: [embed.setDescription(`${list.slice(snipeData.length > sıra ? snipeData.length - sıra : 0, snipeData.length).join('\n')}`)] })
    }
}
exports.config = {
    name: "snipe",
