const ayar = require('../../settings.js');
module.exports.run = async (client, message, args, embed) => {
    if (!message.member.permissions.has("ADMINISTRATOR")) return message.channel.send({ embeds: [embed.setDescription(`${message.author}, Bu komutu kullanmak için yeterli yetkiye sahip değilsin!`)] }).sil(7);
    let everyone = message.guild.roles.cache.get(message.guild.id);
    let permObjesi = {};
    let everPermleri = message.channel.permissionOverwrites.cache.get(everyone.id);
    if (!everPermleri) {
        message.channel.permissionOverwrites.create(everyone, {
            SEND_MESSAGES: false,
        });
        message.channel.send({ embeds: [embed.setDescription(`Kanal kilitlendi!`)] }).sil(7)
    } else {
        everPermleri.allow.toArray().forEach(p => {
            permObjesi[p] = true;
        });
        everPermleri.deny.toArray().forEach(p => {
            permObjesi[p] = false;
        });
        if (message.channel.permissionsFor(everyone).has('SEND_MESSAGES')) {
            permObjesi["SEND_MESSAGES"] = false;
            message.channel.permissionOverwrites.create(everyone, permObjesi);
            await message.channel.send({ embeds: [embed.setDescription(`Kanal kilitlendi!`)] }).sil(7);
        } else {
            permObjesi["SEND_MESSAGES"] = true;
            message.channel.permissionOverwrites.create(everyone, permObjesi);
            await message.channel.send({ embeds: [embed.setDescription(`Kanal kilidi açıldı!`)] }).sil(7);
        };
    }
};
exports.config = {
    name: "kilit",
    usage: `${ayar.bot.botPrefix}kilit`,
    guildOnly: true,
    aliases: ["lock", "kanalkilit"],
    cooldown: 3000
};