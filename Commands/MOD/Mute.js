...
module.exports.run = async (client, message, args, embed) => {
    ...
    if (!member || !zaman || !sebep) return message.channel.send({ embeds: [embed.setDescription(`${message.member}, Hatalı kullanım. Örnek: **${ayar.bot.botPrefix}mute @Cross/ID [Time] [Reason]**`)] }).sil(7)
    ...
    if (durum) return message.channel.send({ embeds: [embed.setDescription(`${member}, Adlı kullanıcının aktif bir **[CHAT-MUTE]** cezası bulunmakta.`)] }).sil(7);
    ...
    setTimeout(async () => {
        ...
        data.forEach(async (d, index) => {
            if (d.Durum == true) {
                ...
                if (client.channels.cache.get(ayar.channels.muteLog)) client.channels.cache.get(ayar.channels.muteLog).send({ embeds: [embed.setDescription(`${member}, Adlı kullanıcının metin kanallarında susturması kalktı.`)] })
                d.Durum = false;
                d.save();
            } else {
                return;
