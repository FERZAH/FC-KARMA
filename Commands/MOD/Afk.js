...
module.exports.run = async (client, message, args, embed) => {
    ...
    if (durum) return message.channel.send({ embeds: [embed.setDescription(`Zaten afk modundasın!`)] }).sil(7);
    let sebep = args.slice(0).join(' ') || "Belirtilmemiş."
    if (sebep && yasaklar.some(s => sebep.toLowerCase().includes(s))) {
        message.channel.send({ embeds: [embed.setDescription(`Afk sebebine **reklam** koyamazsın!`)] }).sil(7)
        message.delete();
    }
    message.channel.send({ embeds: [embed.setDescription(`${message.member}, Adlı kullanıcı **${sebep}** sebebiyle afk oldu!`)] }).sil(7);
