...
module.exports.run = async (client, message, args, embed) => {
    ...
    let taglilar = role.members.filter(s => ayar.guild.tags.some(a => s.user.tag.toLowerCase().includes(a)));
    let rolrenk = role.hexColor;


    await message.channel.send({ content: `${role} Rolündeki seste olmayan kullanıcılar` }).sil(15)
    await message.channel.send({ content: `${online.filter(s => !s.voice.channel).map(a => a).join('\n')}`, split: true, code: "js" }).sil(15)
