...
module.exports.run = async (client, message, args, embed) => {
    ...
    if (member.roles.cache.has(role.id)) {
        ...
        message.channel.send({ embeds: [embed.setDescription(`${member}, Adlı kullanıcıdan ${role} rolü alındı.`)] }).sil(7)
    } else {
        await member.roles.add(role.id).catch(e => { });
        message.channel.send({ embeds: [embed.setDescription(`${member}, Adlı kullanıcıya ${role} rolü verildi.`)] }).sil(7)
    }
};
exports.config = {
    name: "rol",
